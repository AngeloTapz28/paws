...
<style>
    /* ── Info label/value ── */
    .info-label { font-size:.75rem; color:var(--muted); font-weight:500; margin-bottom:.2rem; }
    .info-value { font-size:.9rem; font-weight:700; color:var(--text); }

    /* ── Divider ── */
    .receipt-divider { border:none; border-top:1px solid var(--border); margin:1.25rem 0; }
    ...
    .txn-item {
        ...
        padding:.75rem; border-radius:10px; background:var(--bg);
        ...
        <div class="card card-receipt">
            ...
            <div class="card-body" style="padding:1.5rem;">
                ...
                <div class="receipt-header d-flex align-items-start justify-content-between mb-1">
                    ...
                    @endphp
                    <span class="status-badge {{ $sClass }}">{{ ucfirst($payment->status) }}</span>
                </div>
                ...
                <div class="row g-4" id="infoGrid">
                    ...
                    <div class="col-md-6 info-pair" data-idx="1">
                        ...
                        <div class="info-value">{{ ucwords(str_replace('_', ' ', $payment->method ?? $payment->payment_method ?? '—')) }}</div>
                    </div>
                    ...
                    <div class="col-md-6 info-pair" data-idx="3">
                        ...
                        <div class="info-value">
                            {{ ($payment->paid_at ?? $payment->created_at)?->format('M d, Y h:i A') }}
                        </div>
                    </div>
                    @if($payment->external_reference)
                    <div class="col-md-6 info-pair" data-idx="4">
                        <div class="info-label">External Reference #</div>
                        <div class="info-value" style="font-family:monospace;">{{ $payment->external_reference }}</div>
                    </div>
                    @endif
                    ...
                    <div class="col-md-6 info-pair" data-idx="5">
                        ...
                        <div class="info-value">
                            <a href="{{ route('staff.applications.show', $payment->adoptionApplication) }}"
                    ...
                    <div class="col-12 info-pair" data-idx="6">
                        ...
                        <div class="info-value" style="font-weight:500;">{{ $payment->notes ?? 'none' }}</div>
                    </div>
                </div>
    ...
    <div class="col-lg-4">
        <div class="card card-txn">
            ...
            <div class="card-body" style="padding:1rem 1.25rem;" id="txnList">
                ...
                {{-- Creation entry ── --}}
                <div class="txn-item">
                    <div class="txn-icon"><i class="bi bi-check-lg"></i></div>
                    <div>
                        <div style="font-size:.83rem; font-weight:700; color:var(--navy);">Credit</div>
                        <div style="font-size:.75rem; color:var(--muted);">
                            {{ ($payment->paid_at ?? $payment->created_at)?->format('M d, Y h:i A') }}
                        </div>
                        @if($payment->recordedBy ?? $payment->recorder)
                        <div style="font-size:.75rem; color:var(--muted); margin-top:.15rem;">
                            Recorded by: {{ $payment->recordedBy->name ?? $payment->recorder->name ?? 'Staff' }}
                        </div>
                        @endif
                    </div>
                </div>
                ...
                @if(isset($payment->transactions) && $payment->transactions->count())
                    ...
                    <div class="txn-item">
                        ...
                        <div>
                            ...
                            @if($txn->description)
                            <div style="font-size:.75rem; color:var(--muted); margin-top:.15rem;">{{ $txn->description }}</div>
                            @endif
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="txn-item">
                    <div class="txn-icon"><i class="bi bi-patch-check-fill"></i></div>
                    <div>
                        <div style="font-size:.83rem; font-weight:700; color:var(--navy);">Completed</div>
                        <div style="font-size:.75rem; color:var(--muted);">Payment marked as completed</div>
                    </div>
                </div>
                @endif
